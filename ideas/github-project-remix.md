...
Use one composer on home and profile pages. Select a repository with **Remix**, scroll to the composer, and focus the prompt. Wait for submission before creating repositories or issues. Use `AgentsLoop/PlayGround` with the server credential when no repository is selected. Require sign-in and record the requesting user in the issue body.
