...
`issues.labeled` events that add that label. Comments, edits, and other labels
do not execute the workflow. A local workflow that already handles issue events
runs natively and is not dispatched a second time. A dispatch-only local
wrapper is dispatched with the installation token. A 404 creates a thin local
