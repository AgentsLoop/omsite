...
test('materializes a root-level GitHub Actions build ZIP', async () => {
  ...
  zip.addFile('.omgithub-opencode-log.jsonl', Buffer.from('{"type":"text","part":{"text":"review"}}\n'))
  ...
  assert.equal(existsSync(join(project.local_dir, '.omgithub-opencode-log.jsonl')), false)
  assert.equal(readFileSync(join(gamesDir, '..', 'opencode-logs', `${project.slug}.jsonl`), 'utf8'), '{"type":"text","part":{"text":"review"}}\n')
