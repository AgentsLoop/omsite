...
const graphicsDemandLevels = new Set(['light', 'moderate', 'heavy', 'extreme', 'ultra', 'godlike'])
...
export function validateCatalogMetadata(input) {
  ...
  if (!Array.isArray(input.tags) || input.tags.length > 64 || input.tags.some(tag => typeof tag !== 'string' || tag.length > 100)) fail('tags')
  let graphics_demand = null
  if (input.graphics_demand != null) {
    const estimate = input.graphics_demand
    if (!estimate || typeof estimate !== 'object' || Array.isArray(estimate) ||
        !graphicsDemandLevels.has(estimate.level) || estimate.source !== 'opencode' ||
        typeof estimate.assumptions !== 'string' || !estimate.assumptions.trim() ||
        estimate.assumptions.length > 1500) fail('graphics_demand')
    graphics_demand = { level: estimate.level, source: 'opencode', assumptions: estimate.assumptions.trim() }
  }
  ...
  const evidence = input.metadata_evidence.map(item => {
    if (!item || !['description', 'tags', 'complexity_score', 'prompt', 'graphics_demand'].includes(item.field) || typeof item.file !== 'string' || !item.file || item.file.startsWith('/') || item.file.includes('\\') || item.file.split('/').some(part => part === '..' || part === '.') || !Number.isInteger(item.line_start) || item.line_start < 1 || !Number.isInteger(item.line_end) || item.line_end < item.line_start) fail('evidence reference')
    return { field: item.field, file: item.file, line_start: item.line_start, line_end: item.line_end }
  })
  for (const field of ['complexity_score', ...(graphics_demand ? ['graphics_demand'] : []), ...(input.description_source === 'opencode' ? ['description'] : []), ...(prompt ? ['prompt'] : []), ...(input.tags.length ? ['tags'] : [])]) {
  ...
  return { schema_version: 1, description: input.description.trim(), description_source: input.description_source,
    ...
    graphics_demand, tags: normalizeTags(input.tags), complexity_score: input.complexity_score, metadata_source: input.metadata_source,
