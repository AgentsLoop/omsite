...
export function validateCatalogMetadata(input) {
  ...
  const evidence = input.metadata_evidence.map(item => {
    if (!item || !['description', 'tags', 'complexity_score', 'prompt'].includes(item.field) || typeof item.file !== 'string' || !item.file || item.file.startsWith('/') || item.file.includes('\\') || item.file.split('/').some(part => part === '..' || part === '.') || !Number.isInteger(item.line_start) || item.line_start < 1 || !Number.isInteger(item.line_end) || item.line_end < item.line_start) fail('evidence reference')
    return { field: item.field, file: item.file, line_start: item.line_start, line_end: item.line_end }
  })
  for (const field of ['complexity_score', ...(input.description_source === 'opencode' ? ['description'] : []), ...(prompt ? ['prompt'] : []), ...(input.tags.length ? ['tags'] : [])]) {
  ...
  return { schema_version: 1, description: input.description.trim(), description_source: input.description_source,
    ...
    tags: normalizeTags(input.tags), complexity_score: input.complexity_score, metadata_source: input.metadata_source,
