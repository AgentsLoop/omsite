...
export async function dispatchPublicBuild({
  ...
  while (Date.now() < deadline) {
    if (run) {
      run = await githubJson(`/repos/${encodeURIComponent(workflowOwner)}/${encodeURIComponent(workflowRepo)}/actions/runs/${run.id}`, token, requestFetch, { cache: 'no-store' })
    } else {
    const data = await githubJson(`${workflow}/runs?event=workflow_dispatch&branch=${encodeURIComponent(workflowRef)}&per_page=50`, token, requestFetch, { cache: 'no-store' })
