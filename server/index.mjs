...
import { extractUrls, github, handleActionableIssue, isActionableIssueEvent, verifyWebhookSignature } from './lib/github.mjs'
...
const buildUploadMaxBytes = 1024 * 1024 * 1024
const githubApp = {
...
app.post('/api/github/webhooks', express.raw({ type: 'application/json', limit: '2mb' }), async (req, res, next) => {
  try {
    ...
    if (!isActionableIssueEvent(event, payload)) return res.status(202).json({ accepted: false })
    if (!githubApp.appId || !githubApp.privateKey) {
      return res.status(503).json({ error: 'GitHub App is not configured' })
    }
    const result = await handleActionableIssue(payload, githubApp)
...
app.post('/api/builds', express.raw({ type: ['application/zip', 'application/octet-stream'], limit: `${buildUploadMaxBytes}b` }), async (req, res, next) => {
  ...
  try {
    ...
    const uploadEntry = String(req.headers['x-omgithub-source-entry'] || '')
    const pathMatches = pending.projectPath ? equalSecret(pending.projectPath, uploadPath) : uploadPath === ''
    const entryMatches = pending.sourceEntry ? equalSecret(pending.sourceEntry, uploadEntry) : uploadEntry === ''
...
app.get('/api/github/:owner/:repo/blob/:ref/*path', async (req, res, next) => {
  try {
    const parts = (Array.isArray(req.params.path) ? req.params.path : [req.params.path]).filter(Boolean)
    const isProgress = parts.at(-1) === 'progress'
    const filePath = (isProgress ? parts.slice(0, -1) : parts).join('/')
    if (!filePath) throw Object.assign(new Error('A blob entry file is required'), { status: 400 })
    const sourceEntry = basename(filePath)
    const projectPath = dirname(filePath) === '.' ? '' : dirname(filePath)
    const ref = req.params.ref
    const publicPath = namedPublicPath(req.params.owner, req.params.repo, ref, projectPath, 'blob', sourceEntry)
    const publication = isCommitRef(ref)
      ? await startPublication({ owner: req.params.owner, repo: req.params.repo, sha: ref, projectPath, sourceEntry, publicPath })
      : await startNamedPublication({ owner: req.params.owner, repo: req.params.repo, ref, projectPath, sourceEntry, publicPath })
    const payload = { commit: publication.sha || publication.project?.commit || null, ...publicationPayload(publication) }
    if (isProgress) return res.status(publication.state === 'published' ? 200 : publication.state === 'failed' ? 502 : 202).json(payload)
    const project = await publication.promise
    res.json(storePayload(project))
...
app.get('/api/github/:owner/:repo/progress', async (req, res, next) => {
  try {
    ...
    const cached = await store.byRepositoryPath(req.params.owner, req.params.repo, '', '')
    if (cached?.status === 'published') {
      return res.status(200).json({ commit: cached.commit || null, ...publicationPayload({ state: 'published', project: cached, error: '', runId: '' }) })
    }
