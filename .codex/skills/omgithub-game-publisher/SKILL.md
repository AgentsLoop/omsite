...
5. Extract metadata from evidence. Include a factual title and description, one merged tags array, engine and model tags when the source supports them, and a private complexity score from 1 to 10.
...
7. Inspect screenshots and runtime assets. If screenshots are absent, let the OmGithub build capture one. Do not submit a candidate with an unresolved entry point, missing local assets, or a known runtime failure.
